...
class MainWindow(qtw.QMainWindow):
    ...
    def hematology(self):
        ...
        start_y = 75
        # Heme Validators.
        self.regexp_input_PLT = qtc.QRegExp('([1-9]\d?\d?\d?)(\.\d\d?)?')
        self.regexp_input_WBC = qtc.QRegExp(
            '^(0|1\d?\d?\d?|[1-9]\d?\d?)(\.\d\d?)?$')
        self.regexp_input_RBC = qtc.QRegExp('[0-9](\.\d\d?)?')
        self.regexp_input_HGB = qtc.QRegExp('([0-9]|[1-3]\d?)(\.\d\d?)?')
        self.regexp_input_HCT = qtc.QRegExp('[1-8]\d?')
        self.regexp_input_MCV = qtc.QRegExp('((1[1-3]|[5-9])\d)(\.\d\d?)?')
        self.regexp_input_MCH = qtc.QRegExp('(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_MCHC = qtc.QRegExp('(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_RDW = qtc.QRegExp('(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_MPV = qtc.QRegExp('(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_PER_NEUT = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_PER_LYMPH = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_PER_MONO = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_PER_EOS = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_PER_BASO = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_PER_IG = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_ABS_NEUT = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_ABS_LYMPH = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_ABS_MONO = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_ABS_EOS = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_ABS_BASO = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        self.regexp_input_ABS_IG = qtc.QRegExp(
            '(^\d{1,5}$)|(^\d{1,5}\.\d\d?$)')
        ...
        for test, unit in hematology_tests.items():
            ...
            if 'PLT' in test:
                ...
                lineedit_test.setValidator(validator)
            lineedit_test.textEdited.connect(
                lambda: self.check_heme_input(lineedit=lineedit_test.sender()))
            start_y = start_y + 30
...
# | START OF SELF FUNCTIONS |##########| START OF SELF FUNCTIONS |##########| START OF SELF FUNCTIONS |##########| START OF SELF FUNCTIONS |##########| START OF SELF FUNCTIONS \
 #########  ########  ########  ########  ########  ########  ########  ########  ########  ########  ########  ########  ########  ########  ########  ########  ########  ########  #####
   #####      ####      ####      ####      ####      ####      ####      ####      ####      ####      ####      ####      ####      ####      ####      ####      ####      ####      #####
    ...
    def check_heme_input(self, lineedit):
        if
